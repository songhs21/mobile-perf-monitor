...
## 배경
QA 관련 서적에서 모바일 성능 측정 도구를 접하고 원리가 궁금해졌습니다. 예전 단말부터 CPU/GPU 사용량을 위젯으로 확인할 수 있었기 때문에 데이터 수집 자체는 가능하다는 걸 알고 있었고, 지인이 성능 데이터를 그래프로 시각화해 테스트를 자동화했다는 이야기를 듣고 직접 구현해보기로 했습니다.


직무에서 logcat과 dumpsys를 분석하던 경험을 바탕으로 ADB와 Python으로 시작했고, 단순 지표 수집을 넘어 장시간 반복 테스트에서 메모리 누수 패턴을 재현하고 원인을 추정하는 데까지 확장됐습니다.
