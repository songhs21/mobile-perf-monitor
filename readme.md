# 트릭컬 리바이브 모바일 앱 성능 테스트 자동화 v0.4 
...
이는, 시스템에서 명확히 분류되지 않는 네이티브 메모리 영역에서 누수가 발생하고 있을 가능성이 높다고 판단했습니다. 또한 Unknown 영역은 Android 메모리 분류 상 명확히 추적되지 않는 네이티브 영역이 포함될 수 있어, 해당 영역의 증가를 네이티브 메모리 누수의 주요 근거로 판단했습니다.

최종적으로 Total PSS가 단말의 메모리 임계치에 도달한 시점에서 Signal 9로 프로세스가 종료되었고, 이를 통해 해당 이슈는 GC와 무관한 네이티브 메모리 누수로 인해 OS OOM Killer에 의해 종료된 문제로 결론 내렸습니다.
## 테스트 결과 요약
